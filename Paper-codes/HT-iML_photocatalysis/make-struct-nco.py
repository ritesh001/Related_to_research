...
func_group_i = [ncs,no,ocn,scn,sh,oh]
func_group_i_str = ['NCS','NO','OCN','SCN','SH','OH']
...
def find_u_site(struc):
	...
	coor_new.sort()
	l = len(coor)
	m = coor_new[l-1]; m_1 = coor_new[l-2]; m_2 = coor_new[l-3]
	m1 = coor.index(m); m2 = coor.index(m_1); m3 = coor.index(m_2)
	return [m1, m2, m3]
...
def replace_u_site(b):          ## b = ith element of func_group_i
	...
	a = find_u_site(struc)
	if len(b) == 1:
		struc.replace(a[2], Element(sym[0]))
		struc.remove_sites([a[0],a[1]])
	elif len(b) == 2:
		struc.replace(a[2], Element(sym[0]))
		func_atom_coor = [0,0,0]
		for i in range(2):
			func_atom_coor[i] = struc[a[2]].coords[i] - pos[1][i]
		func_atom_coor[2] = struc[a[2]].coords[2] + pos[1][2]
		...
		for i in range(2):
			func_atom_coor1[i] = struc[a[2]].coords[i] - pos[1][i]
			func_atom_coor2[i] = struc[a[2]].coords[i] - pos[2][i]
		func_atom_coor1[2] = struc[a[2]].coords[2] + pos[1][2]
		func_atom_coor2[2] = struc[a[2]].coords[2] + pos[2][2]
...
for dopants in dop:
	...
	for i in range(len(func_group_i)):
		...
		dir_name = func_group_i_str[i] + '-' + 'NCO-' + dopants
